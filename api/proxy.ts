    ...
    try {
        ...
        switch (action) {
            ...
            case 'generateImage':
                ...
                break;
            case 'searchImages':
                result = await handleSearchImages(payload);
                break;
...
const handleSearchImages = async (payload: any) => {
    const { prompt } = payload;
    if (!prompt) throw new Error("A search prompt is required.");

    // A more direct prompt asking for a specific JSON structure for speed and reliability.
    const searchPrompt = `Find 4 high-quality, photorealistic image URLs for a dream board about: "${prompt}". Respond ONLY with a valid JSON object formatted like this: {"imageUrls": ["url1", "url2", "url3", "url4"]}. Do not include markdown formatting or any other text.`;
    ...
    try {
        // The model should return clean JSON, so we attempt to parse it directly.
        const result = JSON.parse(text);
        if (result.imageUrls && Array.isArray(result.imageUrls) && result.imageUrls.length > 0) {
            // Ensure URLs are valid before returning
            const validUrls = result.imageUrls.filter((url: any) => typeof url === 'string' && url.startsWith('http'));
            if (validUrls.length > 0) {
                return { imageUrls: validUrls.slice(0, 4) };
            }
        }
    } catch (e) {
        console.error("Failed to parse JSON from AI response for image search:", text, e);
        // Fallback to regex if JSON parsing fails, as the model might ignore instructions.
        const urlRegex = /https?:\/\/[^\s"']+/g;
        const imageUrls = text.match(urlRegex) || [];
        if (imageUrls.length > 0) {
            return { imageUrls: Array.from(new Set(imageUrls)).slice(0, 4) };
        }
    }

    // If both methods fail, throw an error.
    console.warn("Image search did not return usable URLs for prompt:", prompt);
    throw new Error("Could not find any images for your search. Please try a different term.");
};
